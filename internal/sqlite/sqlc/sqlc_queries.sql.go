...
SELECT id, ocfl_id, spec, digest_algorithm, inventory_digest, root_path, head, node_id FROM ocfl_index_objects WHERE ocfl_id = ?
`
...
func (q *Queries) ListObjects(ctx context.Context, arg ListObjectsParams) ([]ListObjectsRow, error) {
	rows, err := q.db.QueryContext(ctx, listObjects, arg.ID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
